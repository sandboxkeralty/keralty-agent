...
2. `routers/chat.py` retries a quota-failed turn up to 4 total attempts (jittered 2s/5s/10s backoff, ~17s worst case — observed 429 bursts outlived the original 2s/4s budget) — but only while nothing has streamed yet, to avoid duplicated output. A retried turn re-appends the user message to the ADK session (accepted tradeoff).
...
There is **no per-project quota knob to raise** — Gemini 2.5 on Vertex uses dynamic shared quota (429s are capacity, not a limit you can file a ticket against; the console quota page shows 0%/Unlimited while 429s fire, which is the defining DSQ symptom, not a contradiction); the paid escape hatch is Provisioned Throughput.

**Gemini model names (July 2026): agents run on the ROLLING ALIASES `gemini-flash-latest` / `gemini-pro-latest`** (via `settings.GEMINI_FLASH_MODEL`/`GEMINI_PRO_MODEL` — no agent hardcodes a model string anymore). This is not cosmetic: on 2026-07-09 Google **hard-deprecated `gemini-2.5-flash` and `gemini-2.5-pro` on the AI Studio key path** (404 "no longer available" — while still *listing* them in ListModels), which broke style analysis and would have broken every agent. The aliases currently resolve to `gemini-3.5-flash` / `gemini-3.1-pro-preview` and survive future deprecations; `thinking_budget=0` verified working on them. **Vertex rollback trap**: these aliases do NOT exist on Vertex AI — reverting to `GOOGLE_GENAI_USE_VERTEXAI=1` requires also overriding both model env vars with explicit Vertex model IDs. Separately, the preview TTS model intermittently returns an empty candidate (`finish_reason=OTHER`) — `routers/tts.py` retries up to 3×; the flakiness only affects very short inputs (reply-length texts are reliable).

**Current credential state (July 2026, dev/testing phase): the deployed backend runs on the AI Studio Tier 1 API key, not Vertex.** Cloud Run has `GOOGLE_API_KEY` set and `GOOGLE_GENAI_USE_VERTEXAI` removed — a deliberate, user-approved temporary switch: the Tier 1 key's dedicated limits (150 RPM Pro / 1K RPM Flash) sit nearly idle while Vertex DSQ was throwing ~25 429s/day, so agents, TTS, and RAG all use the key for now. **Voice is the exception and is pinned in code**: `routers/voice.py` calls `get_genai_client(force_vertex=True)` because `gemini-live-2.5-flash-native-audio` is the *Vertex* catalog's Live model and the direct Gemini API's Live catalog differs — the pin makes voice immune to credential switches (verified live under the API-key env). Rollback to all-Vertex for real production = restore `GOOGLE_GENAI_USE_VERTEXAI=1` and remove `GOOGLE_API_KEY` (the `GOOGLE_CLOUD_LOCATION=global` var is still set and only matters on the Vertex path). Layers 1–4 above remain in place and matter again the moment that rollback happens.
...
If you add a 9th sub-agent, give it (see any existing agent file for exact wording): the `# IDIOMA — REGLA PRIORITARIA` block (first section — obeys the per-turn language system note), the `# LÍMITES Y TRANSFERENCIA DE ALCANCE` block (with "generar una imagen" in the examples), the `# DOCUMENTOS ADJUNTOS EN EL CHAT` block (including the `[Imagen adjunta]` lines), the `# CITAS EN ENTREGABLES` block, the `# COMUNICACIÓN CON EL USUARIO` block, and the trailing `{writing_style?}` + `{signature?}` placeholder lines — none is optional boilerplate; all exist specifically because any agent can end up handling any turn. Run the brace-token scan afterwards (allowed tokens: exactly those two).

**Agent anonymity + reply language (July 2026, from use-case testing):** every agent carries a `# COMUNICACIÓN CON EL USUARIO` block — never name internal agents (`AnalysisAgent` etc.) or say "transferir a un agente" in user-visible text (calling `transfer_to_agent` is fine; *naming* it isn't). **Reply language is driven by the UI locale, deterministically**: the frontend sends `locale` ("es"/"en", from next-intl's `useLocale()`) in every `POST /api/chat` body, and `routers/chat.py` appends a per-turn `[System note: …]`/`[Nota de sistema: …]` part naming the target language — **the site language always wins**, even when the user types in the other language (product decision, July 2026; every agent's `# IDIOMA` block says to obey the note above all other signals). Prompt rules alone were NOT enough: with an all-Spanish instruction corpus, English questions still got Spanish answers even with a top-priority `# IDIOMA` rule — the deterministic note is the mechanism that actually works; don't remove it thinking the prompts cover it. `_is_english()` (the old message-language sniff) survives only as the fallback when `locale` is absent (older cached frontend builds). Document lookup precedence: when the user doesn't say where a document lives, the orchestrator routes **KB first, Drive second** (routing table + COMPORTAMIENTO rule, explicitly overriding the ask-for-clarification rule), and agents retry name searches with a single distinctive keyword (underscored filenames like `Digital_Twins.pdf` don't match two-word phrases).
...
- **Tab management (July 2026, EditingAgent)**: `sheets_add_tab` / `sheets_rename_tab` (ungated — additive/non-destructive, but audited) and `sheets_delete_tab` (**code-gated HITL** via `_require_approval`, same as update/append — deleting a tab destroys all its data). All three work only on native Google Sheets: `SheetsService._require_native_sheet` rejects raw `.xlsx`/`.xls` uploads with a "convert to Google Sheets first" error, because `spreadsheets().batchUpdate` doesn't understand raw Excel IDs (openpyxl reading does not help for structural writes). Tab names are resolved to sheet IDs server-side; a wrong name returns the list of real tabs. **EditingAgent's instruction carries a "NOMBRES EXACTOS" block listing every Sheets tool name verbatim** — added after Gemini hallucinated a nonexistent `sheets_append_rows` tool in production (ADK raises `ValueError: Tool ... not found` and the whole turn dies as a generic error when the model invents a tool name; the append-rows tool is `append_spreadsheet_values`).
...
`backend/services/genai_client.py` holds the shared `get_genai_client()` helper (Vertex-if-`GOOGLE_GENAI_USE_VERTEXAI=1`, else API-key-if-`GOOGLE_API_KEY`, else Vertex ADC fallback) — extracted from `routers/voice.py` so both the Live API (STT) and `routers/tts.py` use one client-selection implementation instead of two copies. It takes a `force_vertex: bool` parameter used only by the voice route (see the credential-state note in the Agent pipeline section): the Live model exists in the Vertex catalog specifically, so voice must not follow the env-driven selection.
...
`tools/email_tools.py` — all 9 functions call `GmailProvider` with `_credentials(tool_context)`. EmailAgent has mandatory HITL before `email_send`. **Version choice precedes drafting (July 2026)**: EmailAgent presents its two length variants and waits for the user to pick BEFORE calling `email_draft`/`approval_create`; the approval task names the chosen version (`task_description="Enviar correo (<versión>): …"`, `changes_summary` starts with "Versión elegida: …"), and switching versions after a request exists requires a fresh draft + approval (the stale one must not be sent). `email_draft` also auto-appends the user's active signature (see the Signatures section) and returns `signature_applied`.

**Tracking status lifecycle**: `email_tracking.status` progresses `waiting` → `followup_drafted` (set by `followup_service.generate_followup_draft` on successful draft creation, along with `followup_draft_id`/`followup_at`). Both the dashboard summary and the `email_get_tracking` tool query with `status in ["waiting","followup_drafted"]` — a drafted item stays visible with a status badge ("Borrador creado") instead of silently vanishing, and the dashboard button flips to "Regenerar seguimiento". The dashboard's four indicator tiles double as navigation/filters (Bandeja=all, Críticos=CRITICO filter, Seguimiento=tracking tab), and the inbox has per-priority filter chips.
...
`frontend/app/[locale]/email/page.tsx` calls `GET /api/email/summary?tz=...` on mount and on the "Actualizar" button. **← superseded by Correo Ejecutivo v2 below; `GET /summary` is kept only for older cached frontend builds and can be retired.**

### Correo Ejecutivo v2 (July 2026 — full management center; spec: `docs/propuesta-email-ejecutivo-v2.md`)

**Data model.** Per-user Firestore collection `email_threads` (doc id `{user_id}::{thread_id}` — idempotent upserts, no lookup query) holds each thread's facets (`prioridad`, `requiere_accion`+`accion_tipo`, `esperando_respuesta`, `estado_gestion` nuevo/gestionado/resuelto/pospuesto/respondido), AI content (`resumen` ≤500c, `accion_sugerida` ≤200c, `fecha_limite`), direction/timestamps (`last_outbound_at`/`last_inbound_at`/`last_message_internal_date`, epoch ms), and the incremental cursor `gmail_history_id`. Access layer: `services/email/thread_store.py` — also centralizes ALL `email_tracking` access (the raw `db.collection` calls that used to be copied across 3 files) and the per-user settings on `users/{email}.email_settings` (`window_days` 3–14, `followup_days` 1–14, `digest_email_enabled`, `locale` — written with `set(merge=True)`, same credential-wipe trap as `default_style_id`). Composite indexes: `email_threads(user_id ASC, last_message_internal_date DESC)`, `email_digests(user_id ASC, created_at DESC)` — deployed and curl-verified.

**Scan engine** (`services/email/scan_service.py`): runs on dashboard open/refresh (product decision: no scheduled scan job), 60s cooldown. Lists `in:inbox`+`in:sent after:{epoch}` thread refs via the new paginated `GmailProvider.list_thread_refs` (`threads.list` accepts `q` and returns per-thread `historyId` — comparing it against stored state skips unchanged threads at zero API cost), batch-fetches only changed/new threads (`get_threads_batch`, `BatchHttpRequest` ×25), applies the state machine (`_apply_transitions` — pure function: own-reply→gestionado from ANY surface incl. Gmail itself; reply on waiting thread→respondido; resuelto reopens to nuevo ONLY if re-analysis finds `requiere_accion`), analyzes via `services/email/analysis_service.py` (batched Flash, 15/call, direction-labeled excerpts, shares triage's rubric; failed chunk → previous facets kept), reconciles manual `email_tracking` records (reply detection → `status="responded"`), and `_maintenance_pass` re-derives time-based flags (follow-up threshold crossing, postpone expiry) for ALL in-window docs each scan. **Priority floor (product decision)**: a user-set priority is stored in `user_priority`; AI may only RAISE above it (`ai_reescalated=true`, badge "IA re-escaló"), never lower. **View rules (strict precedence, product decision)**: bandeja = estado nuevo (criticals INCLUDED); criticos = CRITICO & !resuelto; pendientes = requiere_accion & !resuelto & **prioridad != CRITICO**; seguimiento = esperando_respuesta & !pospuesto. `compute_indicators` (backend) and `threadInView` (`frontend/components/email/types.ts`) MUST stay in lockstep.

**API** (`routers/email_v2.py`, same `/api/email` prefix): `POST /scan` streams **SSE** progress frames (`data:{type:progress|done|error}` — worker thread + queue; same frame format ChatWindow parses); `GET /threads` serves stored state only (instant first paint); `PATCH /threads/{id}/state|priority`, `POST /threads/{id}/postpone`, `GET/PUT /settings`, `GET /digest`, `GET /search?q=` (Gmail syntax, joined with stored state, never triggers analysis). **Draft cycle (fase 2)**: `POST /threads/{id}/draft` (`services/email/reply_service.py` — action aceptar/declinar/mas_info/delegar/libre + modifiers shorter/more_formal + language es/en/None=sender's language; default style via `format_style_block`, signature auto-applied, `In-Reply-To`/`References` headers so replies thread for EXTERNAL recipients — threadId alone only threads in the sender's own mailbox; regeneration deletes the previous draft), `PUT /drafts/{id}` (edits persist to the REAL Gmail draft, signature re-applied), `DELETE /drafts/{id}`, and **REST-native HITL send**: `POST /drafts/{id}/request-approval` creates the same task shape as `approval_create` → the UI confirm dialog calls the existing `POST /api/tasks/{id}/approve` → `POST /drafts/{id}/send` runs `find_approved_task` + `consume_task` (verified live: unapproved 403, approved 200, replay 403), audited as `email_send` with `metadata.source=dashboard`.

**Push detection (fase 3)** (`routers/hooks.py`, prefix `/hooks` — deliberately OUTSIDE `_AUTHENTICATED_PREFIXES`; auth is in-app **OIDC**: audience = backend URL, SA email must equal `HOOKS_OIDC_SERVICE_ACCOUNT`, empty setting → 404): Gmail `users.watch` (INBOX+SENT) registers at the end of every scan when expiring <48h and via the daily Scheduler job; Pub/Sub pushes hit `POST /hooks/gmail-push`, which is **idempotent via monotonic `history_id` compare** (Pub/Sub redelivers), processes only the changed thread ids through `scan_service.process_thread_ids`, and ALWAYS returns 200 fast (a non-2xx causes retry storms). `HistoryExpired` (Gmail 404 on old startHistoryId) sets `needs_full_scan` and still 200s. Verified live: a send triggered a push that updated `email_threads` with no dashboard open. **Weekly digest** (`services/email/digest_service.py`, Scheduler Mon 07:00 America/Bogota → `POST /hooks/digest`): sections + Gemini narrative (locale from `email_settings.locale`), stored in `email_digests` (in-app `DigestView`), and emailed via `GmailProvider.send_message` — **the platform's ONE documented HITL exception** (self-send only, system-generated content, per-user opt-out; see `docs/audit-2026-07-remediation.md`; never generalize `send_message` to other recipients).

**Infra (fase 3, created July 2026)**: APIs `pubsub.googleapis.com` + `cloudscheduler.googleapis.com` enabled; topic `gmail-notifications` (with `gmail-api-push@system.gserviceaccount.com` as publisher); SA `keralty-hooks-invoker@keraltysandbox`; push subscription `gmail-notifications-push` → `/hooks/gmail-push` (OIDC); Scheduler jobs `gmail-watch-renew` (daily 06:00) and `email-weekly-digest` (Mon 07:00), both America/Bogota. Env vars on the backend: `HOOKS_OIDC_SERVICE_ACCOUNT`, `HOOKS_OIDC_AUDIENCE`, `GMAIL_PUSH_TOPIC`; config knobs: `EMAIL_SCAN_WINDOW_DAYS=7`, `EMAIL_SCAN_MAX_THREADS=150`, `EMAIL_ANALYSIS_BATCH_SIZE=15`, `EMAIL_SCAN_COOLDOWN_SECONDS=60`.

**Frontend**: `app/[locale]/email/page.tsx` + real components in `components/email/` (`types.ts` view rules, `ThreadCard` with priority-dropdown/badges/state buttons/DraftPanel, `EmailSettings`, `SearchBar`, `DigestView` — the old static mockups were deleted). Flow: mount → `GET /threads` (instant) → `POST /scan` SSE with progress. Opening/reading a card NEVER changes state (product decision) — only replies (detected anywhere) or explicit buttons do.

### Google Slides write operations (template-driven design engine, July 2026)

**Decks are created from one of THREE corporate templates** (July 2026): `SLIDES_TEMPLATE_ID` (Keralty default, currently `18tuz_W3VQF4x3vBlzvVIWGM_lf5c3eyt-ubmXMl82lQ`), `SLIDES_TEMPLATE_ID_PRESIDENCIA_CORP` and `SLIDES_TEMPLATE_ID_PRESIDENCIA_STD` — Google Slides conversions of the **re-themed** pptx in `branding/generated/` (NOT the originals; see the Branding section — the shipped pptx carry a stock Office theme), owned by the sandbox user. `slides.py::resolve_template_id` maps the keys; VisualAgent picks per request wording (see Branding). `SlidesService.create_presentation(template_key=...)` copies the chosen deck and deletes its stock slides; any template failure logs `TEMPLATE COPY FAILED` and falls back to a blank default-themed deck. **To swap branding later**: re-run `backend/scripts/retheme_templates.py` (palette+font into `branding/generated/`), then `backend/scripts/upload_slides_template.py` (loops all three: uploads+converts, probes/classifies layouts, empirically tests one slide per layout on a scratch copy, prints the combined gcloud env command) — and LOOK at thumbnails before switching the env vars.
...
`tools/image_tools.py` — generates **16:9** (`IMAGEN_ASPECT_RATIO`) corporate images. **Engine order since 2026-07-17 (Gemini path): Nano Banana first** (`_NANO_BANANA_MODELS` = `gemini-3.1-flash-image` → `gemini-2.5-flash-image`, via `get_genai_client().models.generate_content` with `response_modalities=["IMAGE"]` + `ImageConfig(aspect_ratio=...)`) — switched because Imagen 3 shipped garbled in-image text on a live infographic while Nano Banana renders text correctly (probed + verified on the backend's AI Studio key, which also serves Imagen 4 — the Vertex/ADC path does NOT); returns JPEG (upload honors the mime, `.jpg` blob). The Vertex Imagen `:predict` REST chain below remains the fallback (ADC-based, survives an API-key removal). Two steps:
...
### Skills (developer-provided capability packages, July 2026)

`backend/skills/<skill_id>/SKILL.md` (+ optional `references/*.md`) — SKILL.md-style packages that raise output design quality, loaded once at import by `services/skill_registry.py` (must never import from `agents/`; agent modules read it while building INSTRUCTION). **Hybrid loading**: the body's `## Core` section (soft cap ~1200 chars) is baked into the instruction of each agent in frontmatter `agents:` via `core_block_for_agent(name)` (see the concatenation seam in `agents/visual_agent.py`); the FULL body + references is consumed deterministically in code by tools in frontmatter `tools:` via `full_guidance_for_tool(name)` — first consumer: `tools/image_tools.py::_enrich_prompt`, where skill guidance (when present) becomes the primary prompting craft with `brand.IMAGE_STYLE_DIRECTIVE` always appended as the brand color/mood layer (log line `[image_generate] art direction source: skill|brand-fallback` tells you which path ran). All skill text is brace-sanitized (`{`→`(`) at load — the brace-trap rule applies to skills like any instruction text. A malformed skill logs `SKIPPED` and never crashes startup; no feature flag — absent files = disabled. Validator: `cd backend && python -m services.skill_registry` (exit 1 on hard error; run it before committing a skill). Adding a skill that targets already-wired agents/tools is a content-only change (new dir + redeploy). **Provider scoping**: frontmatter `providers: [openai]` limits a skill to conversations on that chat provider — the vocabulary is `model_registry.ModelSpec.provider` (`google`/`anthropic`/`openai`, NOT "gemini"/"claude"; this mismatch shipped once and silently disabled the skill) — and `image_generate` derives it from `state["model_provider"]` (anthropic→the Gemini/Imagen path). `tool_references: [x.md]` picks which references feed `full_guidance_for_tool` (excludes pure API-code refs; `scripts/` and non-.md files are ignored/reserved). Current image skills: `image-gen-pro` (providers: [openai], gpt-image-2 prompt craft; its generic `## Core` is the one baked into VisualAgent) and `gemini-api-image-gen` (providers: [google, anthropic], Nano Banana prompting guide). Their editing templates and API params (`size`/`quality`/`n`, model selection, grounding) are deliberately non-actionable (platform generates only; `image_generate` exposes no params) and the enricher meta-prompt says to ignore them. The enricher also hard-codes an ADULTS-ONLY rule (never prompt for children/minors): the project's Imagen person-generation setting is "Allow (Adults only)" and its RAI filter silently returns an empty/filtered prediction for images containing minors — `image_generate` logs `safety-filtered/empty (reason)` (via `includeRaiReason`) and retries once per model (the filter is nondeterministic; observed live). A deliberately deferred `load_skill` tool (progressive disclosure for future presentation/doc skills) is designed in `backend/skills/README.md`'s flow — the registry API already supports it. Never target a skill at all 9 agents (routing-independent rules belong in `BRAND_INSTRUCTION_BLOCK`).
...
### Writing style personalization (per-executive)

Executives personalize the tone/style of generated content via the "Estilos de escritura" page (`frontend/app/[locale]/estilos/page.tsx`, Sidebar link) and a `PenLine` selector in the chat composer. Two style sources: **4 code-shipped presets** (`backend/services/style_presets.py`, ids `preset:*` — read-only, the router 400s writes to them) and **custom styles** distilled from the executive's own sample documents (Firestore `writing_styles` collection, user-scoped like `email_tracking`, always ownership-checked; foreign/missing ids resolve to *no style*, never an error). Capture flow: upload up to 5 samples → `POST /api/style/analyze` (reuses `extract_text()`; nothing persisted) → one Gemini Flash call (`style_service.analyze_samples`, `thinking_budget=0`) returns a ≤2000-char Spanish style guide → **the executive reviews/edits the guide in a textarea and approves it** → `POST /api/style` saves. Default style lives on `users/{email}.default_style_id` (written with `set(..., merge=True)` — anything else risks nuking stored credentials). Router: `backend/routers/style.py` mounted at `/api/style` (auto-covered by auth middleware); write ops audited.

**Injection mechanism — ADK instruction templating.** Agents end their INSTRUCTION with bare `{writing_style?}` and `{signature?}` lines (originally only Writing/Email/Editing had the style placeholder — extended in July 2026 after sticky routing let un-styled agents, e.g. KnowledgeAgent, draft replies that ignored the selected tone). **Exception: VisualAgent deliberately has NO `{writing_style?}` (2026-07-17)** — its deliverables (images, slides) are visual, and the injected style guide kept leaking letter-format essays ("Dear team,") into image delivery on Gemini even with explicit scoping rules in both the preamble and the agent instruction (the concrete guide text + styled session history beat abstract scoping); removing the block from that agent's context is the deterministic fix. It keeps `{signature?}`. ADK interpolates `{key}` placeholders in plain-string instructions from session state (`inject_session_state`); the trailing `?` makes it optional (empty when absent). **TRAP: a bare `{identifier}` without `?` in ANY agent instruction raises KeyError at runtime and kills every turn that agent handles** — never add brace tokens to instructions (JSON examples included); there's a pre-flight scan pattern in the git history. The heading + precedence preamble live inside the state *value* (`format_style_block`), so an empty value renders nothing; the preamble explicitly subordinates the style to GUARDRAILS/approval flows, and user-editable guide text gets `{}`→`()` replaced defensively. **Scope (2026-07-17, user decision): the style applies ONLY to deliverables** (correos, documentos, comunicados, memos, presentaciones) — the preamble explicitly excludes conversational chat replies, which stay neutral; before this, a default style leaked letter-format greetings ("Dear team,") into every chat answer.

**Two load-bearing invariants in `routers/chat.py`:**
...
### Attaching documents in chat (Drive + local upload + drag & drop + images)

Paperclip button in `ChatWindow.tsx` opens `<AttachMenu>` (`components/documents/AttachMenu.tsx`), a small two-row menu: "Upload from device" and "Google Drive." **Files can also be dragged and dropped anywhere onto the chat window** (`onDragEnter/Over/Leave/Drop` on the ChatWindow root, a depth counter ref so child elements don't flicker the overlay, `documents.dropHint` overlay) — drops route through the same `uploadLocalFile()` helper the paperclip uses. Both sources converge on the same `attachedDocs: { file: DriveFile; text: string; imageBase64?: string }[]` state (**up to 5 files per message, mixing both sources**; `appendAttachment` dedupes by file id and surfaces a localized `documents.tooManyFiles` error past the cap) — a locally uploaded file is wrapped in a synthetic `DriveFile`-shaped object (`id: "local:<uuid>"`). The chips are individually removable and carry a **source-distinct icon** (lucide `Image` = image, `Cloud` = Google Drive, `HardDrive` = device upload) — the `local:` id prefix is what distinguishes upload sources everywhere.

**Image attachments (multimodal input, July 2026).** PNG/JPEG/WebP attach as *real inline image parts* the model can see (`types.Part.from_bytes`, preceded by a `[Imagen adjunta: <name>]` text marker every agent's adjuntos block explains). Local images never hit `/documents/upload` — the frontend reads them as base64 data-URLs client-side; Drive images come back as `{image_base64, mime_type}` from `GET /documents/{id}/text` (which special-cases `image/*` mimeTypes). Caps: 10 MB input, **max 3 images per message** (enforced frontend + backend). **`routers/chat.py::_compress_image` (Pillow) is load-bearing, not cosmetic**: the user-message ADK Event persists to Firestore with inline bytes base64-encoded inside `event_json`, and the 1 MB/doc limit means uncompressed images would make `_write_event` fail silently and drop that turn from restored history — images are downscaled to ≤1024px JPEG stepping down until ≤200 KB each (3 × 200 KB ≈ 800 KB encoded: safe). Image bytes are also deliberately kept out of `session.state["attached_documents"]`. Image *editing* is not supported — analysis only; `drive_read` (the agent tool) still rejects images, since agents receive them via chat attachments. Note the model re-reads session history every turn, so attached images cost input tokens on every subsequent turn of that session.

**Drive picker folder navigation (July 2026).** `DriveService.list_documents` gained a `folder_id` param: browse mode queries `'<id>' in parents` including `application/vnd.google-apps.folder` children (`orderBy="folder,modifiedTime desc"`, folders first, `trashed=false`); a name `query` always takes global-search precedence (no parents clause) — that's also the picker's search-box semantics. `DocumentPicker.tsx` defaults to browsing `folder_id="root"` ("Mi unidad") with a breadcrumb trail; clicking a folder descends, clicking a crumb truncates back, searching shows global flat results. **Limitation**: the tree shows My Drive only — shared-with-me files are reachable via search, not browsing. `_DEFAULT_MIME_TYPES` now also includes `image/png|jpeg|webp|gif`, so both the picker and `drive_search` find images.
...
Either way, the files are sent as `attached_files: [{text, file_id, file_name, mime_type}]` in the chat request body (the legacy single-attachment fields `attached_context`/`attached_file_id`/`attached_file_name`/`attached_mime_type` are still accepted and normalized into the list — `ChatRequest.normalized_attachments()` — so an older cached frontend build keeps working; server caps at 5 files / 8000 chars each). Each file becomes its **own message part**: `[Documento adjunto: <name>]` plus, for real Drive files, a `[drive_file_id: <id> | mimeType: <type>]` line so agents can operate on the *file itself* (edit the Sheet, extend the Doc/Slides via their tools) rather than only its extracted text — every agent's `# DOCUMENTOS ADJUNTOS` block explains this, and the orchestrator routes modification requests by type (EditingAgent for Docs/Sheets, VisualAgent for Slides). The `drive_file_id` line is **deliberately omitted for local uploads** (their synthetic `local:` id is filtered in `chat.py`): the file doesn't exist in Drive, and including an id would bait agents into Drive calls that must fail. **Attachments must be message parts, not just `session.state["attached_documents"]`** — an earlier version only wrote to session state and never read it back out anywhere (no agent instruction or tool referenced that key), so attaching a document silently never reached the model at all, for either source — confirmed by testing the full round-trip, not assumed. `attachedDocs` is cleared after a successful send (it previously wasn't, so every subsequent turn re-appended the same text into session state, growing it unboundedly for the whole session).
...
`GOOGLE_GENAI_USE_VERTEXAI=1` makes ADK use Vertex AI; without it, ADK requires `GOOGLE_API_KEY`. **Currently the deployed backend runs on `GOOGLE_API_KEY` (the flag is removed) as a temporary dev/testing measure** — see the credential-state note in the Agent pipeline section before changing either var.
...
- **Firestore**: default database (`us-central1`) — collections:
  ...
  - `email_tracking` — follow-up tracking records (manual chat flow; reconciled into `email_threads` at scan time)
  - `email_threads` — Correo Ejecutivo v2 per-user thread state (facets, summaries, incremental cursor)
  - `email_digests` — weekly digest documents (v2 fase 3)
  - `kb_chunks` — RAG document chunks with 768-dim embedding arrays
  - `kb_documents` — RAG document metadata
- **Firestore composite indexes** (deployed): `email_tracking(user_id,status)`, `tasks(user_id,status,created_at)`, `messages(session_id,timestamp)`, `sessions(user_id,updated_at)`, `email_threads(user_id,last_message_internal_date DESC)`, `email_digests(user_id,created_at DESC)` — **double-check the actual field name a query orders by before assuming an index matches it.** Two of these were originally deployed against the wrong field (`created_at` instead of `timestamp`/`updated_at`), which silently 500'd conversation history for an unknown period — the frontend's `if (res.ok)` checks swallowed the error, so it just looked like "no data" in the UI, not a bug.
...
Enabled APIs: Drive, Docs, Sheets, Slides, Gmail, Vertex AI, Gemini, Firestore, Cloud Run, Artifact Registry, Cloud Build, Logging, Monitoring, Trace, **Pub/Sub + Cloud Scheduler (enabled July 2026 for Email v2 fase 3 — topic `gmail-notifications`, subscription `gmail-notifications-push`, SA `keralty-hooks-invoker`, jobs `gmail-watch-renew` + `email-weekly-digest`)**. **Not enabled** (verified directly, despite being assumed in earlier planning docs): Calendar, Cloud DLP — both are prerequisites for specific items in `docs/product-roadmap-new-features.md` and need enabling (plus, for Calendar, a new OAuth scope + user re-consent) before that work starts.
