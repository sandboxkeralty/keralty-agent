...
**Agent anonymity + reply language (July 2026, from use-case testing):** every agent carries a `# COMUNICACIÓN CON EL USUARIO` block — never name internal agents (`AnalysisAgent` etc.) or say "transferir a un agente" in user-visible text (calling `transfer_to_agent` is fine; *naming* it isn't), and reply in the user's language. **Prompt rules alone were NOT enough for language**: with an all-Spanish instruction corpus, English questions still got Spanish answers even with a top-priority `# IDIOMA` rule — the working fix is deterministic: `routers/chat.py::_is_english()` sniffs the user message and appends a `[System note: … reply must be in English]` part when English is detected. Don't remove that thinking the prompts cover it; they demonstrably don't. Document lookup precedence: when the user doesn't say where a document lives, the orchestrator routes **KB first, Drive second** (routing table + COMPORTAMIENTO rule, explicitly overriding the ask-for-clarification rule), and agents retry name searches with a single distinctive keyword (underscored filenames like `Digital_Twins.pdf` don't match two-word phrases).
