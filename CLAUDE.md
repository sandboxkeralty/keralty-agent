...
If you add a 9th sub-agent, give it the `# LÍMITES Y TRANSFERENCIA DE ALCANCE` block, the `# DOCUMENTOS ADJUNTOS EN EL CHAT` block, the `# IDIOMA — REGLA PRIORITARIA` block (first section), and the `# COMUNICACIÓN CON EL USUARIO` block (see any existing agent file for the exact wording) — none is optional boilerplate; all exist specifically because any agent can end up handling any turn.
